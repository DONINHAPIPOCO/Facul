...
class Personagem{
    ...
    public static boolean pesquisarPorID(String id, ArrayList<Personagem> array){
        boolean resp = false;
        for(int i = 0; i < array.size(); i++){
            if(array.get(i).getId().equals(id)) resp = true;
...
public class PilhaFlexivel{
    ...
    public void empilhar(Personagem elemento) throws Exception{
        if(elemento != null){
            if(this.getTopo() == null){
                this.setTopo(new Celula(elemento));
    
            } else {
                Celula tmp = this.getTopo();
                this.setTopo(new Celula(elemento));
                this.getTopo().setProx(tmp);
                tmp = null;
    
            }
        } else {
            throw new Exception("ERRO AO INSERIR! Objeto Nulo!!!");
    ...
    public static void main(String[] args) throws ParseException{
        ...
        HashMap<String, Personagem> map = new HashMap<>();

        for (Personagem personagem : array) {
            map.put(personagem.getId(), personagem);
