...
public class Insercao {
    ...
    public static void insertionSort(Personagem[] array){
        for(int i = 1; i < array.length; i++){
            int j = i - 1;
            Personagem tmp = array[i];

            while(j >= 0 && (tmp.getDateOfBirth().before(array[j].getDateOfBirth()) 
            || (array[j].getDateOfBirth().equals(tmp.getDateOfBirth()) 
