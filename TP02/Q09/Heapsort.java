...
public class Heapsort {
    ...
    public static void heapSort(Personagem[] array){
        Personagem[] heap = new Personagem[array.length + 1];
        for(int i = 0; i < array.length; i++){
            heap[i+1] = array[i];
        ...
        for(int tamanho = 2; tamanho < array.length; tamanho++){
            construir(tamanho, array);

        }

        int tamanho = array.length-1;
        while(tamanho > 1){
            swap(array, 1, tamanho--);
            reconstruir(tamanho, array);

        }

        heap = array;
        array = new Personagem[array.length];
    ...
    public static void construir(int tamanho, Personagem[] array){
        for(int i = tamanho; i > 1 && array[i].getHairColour().compareTo(array[i/2].getHairColour()) < 0; i /= 2){
            System.out.println("CHAMANDO SWAP NO CONSTRUIR:");
            swap(array, i, i/2);
    ...
    public static void reconstruir(int tamanho, Personagem[] array){
        int i = 1;
        while(i <= (tamanho / 2)){
            int filho = getMaior(array, i, tamanho);
            if(array[i].getHairColour().compareTo(array[filho].getHairColour()) < 0){
                System.out.println("CHAMANDO SWAP NO RECONSTRUIR:");
                swap(array, i, filho);
                i = filho;
    ...
    public static int getMaior(Personagem[] array, int i, int tamanho){
        int filho;
        if (2*i == tamanho || array[2*i].getHairColour().compareTo(array[2*i+1].getHairColour()) > 0){
            filho = 2*i;
        } else {
            filho = 2*i + 1;
        }
    ...
    public static void swap(Personagem[] array, int i, int j){
        System.out.println("SWAP: " + array[i].getName() + "[" + i + "]" + " AND: " + array[j].getName() + "[" + j + "]");
    ...
    public static void main(String[] args) throws ParseException{
        ...
        int numEntrada = 0, numEntradaPesquisa = 0;
        ...
        for(int i = 0; i < numEntrada; i++){
            ...
            System.out.println("PERSONAGEM INSERIDO: " + personagem.getName() + " NA POSICAO: " + i);
