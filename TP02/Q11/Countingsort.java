...
public class Countingsort {
    ...
    public static void countingSort(Personagem[] array) {
        ...
        Personagem[] ordenado = new Personagem[array.length];

        for(int i = 0; i < count.length; count[i] = 0, i++);

        for(int i = 0; i < array.length; count[array[i].getYearOfBirth()]++, i++);

        for(int i = 1; i < count.length; count[i] += count[i-1], i++);

        for(int i = array.length - 1; i >= 0; ordenado[count[array[i].getYearOfBirth()]-1] = array[i], count[array[i].getYearOfBirth()]--, i--);

        for(int i = 0; i < ordenado.length; i++) {
            for(int j = i - 1; j >= 0; j--) {
                if (ordenado[j].getYearOfBirth() == ordenado[i].getYearOfBirth() && ordenado[j].getName().compareTo(ordenado[i].getName()) > 0) {
                    //System.out.println("TROCANDO: " + ordenado[j].getName() + " COM: " + ordenado[i].getName());
                    Personagem temp = ordenado[j];
    ...
    public static int getMaior(Personagem[] array) {
